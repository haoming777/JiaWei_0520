    ...
    public class RoundnessDetector
    {
       static XLToolClass toolClass = new XLToolClass();
		...
		public static DetectionResult DetectRoundnessAndRect(Mat img)
        {
			...
			toolClass.SaveLog($"找圆 前处理用时：{stopwatch.ElapsedMilliseconds}ms");
            stopwatch.Restart();
			// 二值化处理，白色区域为255
			Mat binary = new Mat();
            Cv2.Threshold(imgGray, binary, _Config.Camera3Thresh, _Config.Camera3Maxval, ThresholdTypes.Binary);
			toolClass.SaveLog($"找圆 二值化处理用时：{stopwatch.ElapsedMilliseconds}ms");
			stopwatch.Restart();
			...
			toolClass.SaveLog($"找圆 查找所有轮廓用时：{stopwatch.ElapsedMilliseconds}ms");
			stopwatch.Restart();
			...
			toolClass.SaveLog($"找圆 处理结果用时：{stopwatch.ElapsedMilliseconds}ms"); 
        ...
        public static Mat VisualizeDetection(Mat img, DetectionResult result, string savePath = null)
        {
			...
			//}
			toolClass.SaveLog($"找圆 画图用时：{stopwatch.ElapsedMilliseconds}ms");
			stopwatch.Stop();
			return resultImg;
        }
    }
