...
namespace VisionMeasure
{
	...
	int _localTotal = 0; // 本地计数器
		...
		private void InitializeImageProcessors()
		{
			try
			{
				...
				toolClass.SaveLog($"图像处理器初始化完成（{activeCount}/5 启用）");
			...
			try
			{
				...
				if (_cameraEnabled[0])
				{
					...
					catch (Exception ex)
					{
						...
						if (IFSaveLog) toolClass.SaveLog("[CamCfg] Camera1 连接失败: " + ex.Message);
					}
				}
				else {  }
				...
				if (_cameraEnabled[1])
				{
					...
					catch (Exception ex)
					{
						...
						if (IFSaveLog) toolClass.SaveLog("[CamCfg] Camera2 连接失败: " + ex.Message);
					}
				}
				else {  }
				...
				if (_cameraEnabled[2])
				{
					...
					catch (Exception ex)
					{
						...
						if (IFSaveLog) toolClass.SaveLog("[CamCfg] Camera3 连接失败: " + ex.Message);
					}
				}
				else {  }
		...
		private void ProcessCamera3Image(ImageProcessingContext context)
		{
			...
			try
			{
				...
				if (IFSaveLog)
				{
					if (RunLogEnabled) toolClass.SaveLog($"[Camera3] ID:{id} 最终结果 - ProcessResult:{result}, SequenceId:{context.SequenceId}, Offset:{context.Offset}");
				}
			...
			try
			{
				...
				if (rsp_segmentation != null && rsp_ocr != null)
				{
					...
					if (segmentationCount > 0)
					{
						...
						if (RunLogEnabled) toolClass.SaveLog($"[Camera4] ID:{id} 字符检测 - IndexOCR:{index_ocr}, StandChar:{camera4StandChar}, ResultStr:{result_Char_str}, ResultChar:{result_char}");
				...
				if (RunLogEnabled) toolClass.SaveLog($"[Camera4] ID:{id} 处理完成 - result_char:{result_char}, result:{result}");
		...
		private void ProcessCamera5Image(ImageProcessingContext context)
		{
			...
			try
			{
				...
				if (!result && result_char && result_PCode_char && result_flaw && result_Segmentation)
				{
					...
					toolClass.SaveLog($"[Camera5] ID:{id} ⚠ 兜底触发! 综合NG但无缺陷分类, 原始标签:[{label_str}], result_class:[{result_class}]");
					result = false;
				}
				else if (!result && !result_flaw && string.IsNullOrEmpty(result_class))
				{
					toolClass.SaveLog($"[Camera5] ID:{id} ⚠ 缺陷模型标记NG但result_class为空! 原始标签:[{label_str}], result_Class_str:[{result_Class_str}]");
				...
				context.ProcessResult = result;
				if (RunLogEnabled) toolClass.SaveLog($"[Camera5] ID:{id} 最终结果 - result_char:{result_char}, result_PCode_char:{result_PCode_char}, result_flaw:{result_flaw}, result_Segmentation:{result_Segmentation}, FinalResult:{result}");
				...
				if (!_isClosing)
				{
					...
					{
						if (RunLogEnabled) toolClass.SaveLog($"[Camera5] 积压{queueDepth}帧，跳过显示提效");
					}
				...
				var cam5Result = QueueResultItem.Rent()
				{
			...
			try
			{
				...
				if (IFSaveLog)
				{
					...
					for (int ci = 0; ci < results.Length; ci++)
					{
						...
						toolClass.SaveLog("[ResultMatch] " + camNames[ci] + ": Seq=" + r.SequenceId + " Offset=" + r.Offset + " Result=" + r.Result + " " + marker);
					}
				}
			...
			if (unifiedId % 100 == 0) { try { if (FastLogger.IsInitialized) FastLogger.Instance.Debug("里程碑: ID=" + unifiedId + " FinalResult=" + (finalResult?"OK":"NG") + " Total=" + _Config.total + " OK=" + _Config.ok); } catch { } }
				...
				if (modbusClass.modbusState && !_isClosing)
				{
					lock (SendResultList)
					{
						...
						SendResultList.Add(QueueResultItem.Rent()
						{
		...
		private void FlushPendingRecords()
		{
			...
			if (flushList != null && _dbRecorder != null)
			{
				...
				if (_Config.burstExcludeCount > 0 || flushList.Any(r => r.IsExcluded))
				{
					...
					{
						try { this.Invoke(new Action(() => { if (BaoGuanNGTxt != null) BaoGuanNGTxt.Text = _Config.burstExcludeCount.ToString(); }));
						} catch { }
					}
				}
			}
			...
			try
			{
				...
				try
				{
					if (g_handle != IntPtr.Zero)
					{
						...
						toolClass.SaveLog("运动控制卡已断开");
					}
				}
				catch (Exception ex) { toolClass.SaveLog($"断开运控卡异常: {ex.Message}"); }
				...
				for (int i = 0; i < cameras.Length; i++)
				{
					...
					try
					{
						...
						if (!task.Wait(CAM_TIMEOUT))
							toolClass.SaveLog($"⚠ {names[i]} StopStreamGrabber 超时({CAM_TIMEOUT}ms)，跳过");
					}
					catch (Exception ex) { toolClass.SaveLog($"{names[i]} StopStreamGrabber 异常: {ex.Message}"); }
				}
			...
			try
			{
				...
				for (int i = 0; i < cameras.Length; i++)
				{
					...
					try
					{
						...
						if (!task.Wait(CAM_TIMEOUT))
							toolClass.SaveLog($"⚠ {names[i]} Close 超时({CAM_TIMEOUT}ms)，跳过");
					}
					catch (Exception ex) { toolClass.SaveLog($"{names[i]} Close 异常: {ex.Message}"); }
				}
		...
		private void DisposeAIModels()
		{
			try
			{
				...
				toolClass.SaveLog("AI模型已释放");
			}
			catch (Exception ex)
			{
				toolClass.SaveLog($"释放AI模型异常: {ex.Message}");
			}
		}
		#endregion
		...
		private async void clearBtn_Click(object sender, EventArgs e)
		{
			...
			// 仅检查已启用工位的图片路径
			if (_cameraEnabled[0] && !File.Exists(imagePath1)) { MessageBox.Show($"Camera1 图片不存在:\n{imagePath1}"); return; }
			if (_cameraEnabled[1] && !File.Exists(imagePath2)) { MessageBox.Show($"Camera2 图片不存在:\n{imagePath2}"); return; }
			if (_cameraEnabled[2] && !File.Exists(imagePath3)) { MessageBox.Show($"Camera3 图片不存在:\n{imagePath3}"); return; }
			if (_cameraEnabled[3] && !File.Exists(imagePath4)) { MessageBox.Show($"Camera4 图片不存在:\n{imagePath4}"); return; }
			if (_cameraEnabled[4])
			{
			...
			clearBtn.Text = "停止模拟";
			int enabledCount = _cameraEnabled.Count(x => x);
			toolClass.SaveLog($"模拟运行启动，{enabledCount}个工位，3个/组间隔10ms");
		...
		private void Test1Method()
		{
			try
			{
				...
				if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
				{
					...
					// 按启用顺序分配图片到对应相机
					int fileIdx = 0;
					if (_cameraEnabled[0]) { imagePath1 = dialog.FileNames[fileIdx]; OnCamera1Image(new Bitmap(imagePath1), null, null); fileIdx++; }
					if (_cameraEnabled[1]) { imagePath2 = dialog.FileNames[fileIdx]; OnCamera2Image(new Bitmap(imagePath2), null, null); fileIdx++; }
					if (_cameraEnabled[2]) { imagePath3 = dialog.FileNames[fileIdx]; OnCamera3Image(new Bitmap(imagePath3), null, null); fileIdx++; }
					if (_cameraEnabled[3]) { imagePath4 = dialog.FileNames[fileIdx]; OnCamera4Image(new Bitmap(imagePath4), null, null); fileIdx++; }
					if (_cameraEnabled[4]) { imagePath5 = dialog.FileNames[fileIdx]; OnCamera5Image(new Bitmap(imagePath5), null, null); fileIdx++; }
				}
				return;
			}
			catch (Exception ex)
	...
	#region 原有辅助类（保持兼容）
		public class QueueResultItem : IDisposable
		{
			...
			private void Reset()
			{
				SequenceId = 0; Offset = 0;
				ImageData_Y = null; ImageData_R = null;
				Result = false; Timestamp = DateTime.Now;
				IsPureBurst = false;
				Cam5_CharResult = 1; Cam5_PCodeResult = 1; Cam5_SebiaoResult = 1;
				Cam5_BaoguanResult = 1; Cam5_XiekouResult = 1; Cam5_WeijianduanResult = 1;
				IsEmptyCup = false;
				StageTimes?.Clear();
			}
	{
			...
			try
			{
				...
				finally
				{
					...
					if (context.ProcessingTimer != null)
					{
						...
						if (RunLogEnabled) toolClass.SaveLog(stringBuilder.ToString());
						...
						// 每50帧输出性能汇总（仅调试模式）
						if (RunLogEnabled && _performanceStats.ProcessCount > 0 && _performanceStats.ProcessCount % 50 == 0)
						{
	...
	public class ResultMatcher : IDisposable
	{
		...
		private long _debugFrameCount = 0;           // FastLogger调试帧计数
		private readonly int _activeProcessorCount; // 实际启用的处理器数
		private readonly int _firstActiveIndex;     // 第一个启用的处理器下标（锚点）
		...
		public ResultMatcher(ImageProcessor[] processors, Action<QueueResultItem[]> matchCallback)
		{
			...
			{
				toolClass.SaveLog("[ResultMatcher] 警告：所有处理器均未启用，匹配器不会触发");
				_disposed = true; // 标记为已释放，线程不会启动
				return;
			}

			toolClass.SaveLog($"[ResultMatcher] {_activeProcessorCount}/{_processors.Length} 个处理器启用，锚点下标={_firstActiveIndex}");
		...
		private void MatchingWorker()
		{
			...
			try
			{
				while (!_disposed)
				{
					...
					for (int i = 0; i < _processors.Length; i++)
					{
						if (_processors[i] == null) continue; // 禁用工位跳过
		...
		private void PerformMatching()
		{
			...
			try
			{
				...
				if (RunLogEnabled) toolClass.SaveLog("[ResultMatch] 锚点=" + anchorCamName + " RawSeq=" + anchorResult.SequenceId + " Offset=" + anchorResult.Offset + " TargetID=" + targetSequenceId);

				var matchedResults = new QueueResultItem[_processors.Length];

				// 初始化：禁用槽位填虚拟 OK，启用槽位待匹配
				for (int i = 0; i < _processors.Length; i++)
				{
					if (_processors[i] == null)
						matchedResults[i] = CreateVirtualOkResult(targetSequenceId);
					else
				...
				for (int i = 0; i < _processors.Length; i++)
				{
					if (i == _firstActiveIndex) continue;
					if (_processors[i] == null) continue; // 禁用工位已有虚拟 OK
				...
				if (allMatch)
				{
					...
					// 防呆：确保 results 数组 5 个元素全部非 null
					...
					for (int ri = 0; ri < matchedResults.Length; ri++) {
						if (matchedResults[ri] == null) resultSummary += "Cam" + (ri+1) + "=NULL ";
						else {
							long rid = matchedResults[ri].SequenceId - matchedResults[ri].Offset;
							resultSummary += "Cam" + (ri+1) + "=" + rid + "(" + (matchedResults[ri].Result ? "OK" : "NG") + ") ";
						}
					}
					if (RunLogEnabled) toolClass.SaveLog("[ResultMatch] ID:" + targetSequenceId + " 匹配成功 → " + resultSummary);

					// 调试：每 200 帧输出一次匹配统计
					_debugFrameCount++;
					if (_debugFrameCount % 200 == 0)
					{
						try { if (FastLogger.IsInitialized) FastLogger.Instance.Debug("周期报告:已处理" + _debugFrameCount + "帧, 锚点=" + anchorCamName); } catch { }
					}
				...
				{
					// 检查是否有启用的工位已经超过 target（等待也没用，丢弃）
					bool anyPast = false;
					for (int i = 0; i < _processors.Length; i++)
					{
						if (_processors[i] == null) continue;
					...
					if (anyPast)
					{
						toolClass.SaveLog("[ResultMatch] ID:" + targetSequenceId + " 匹配失败(超时丢弃)，锚点=" + anchorCamName);
						// 丢弃锚点结果
